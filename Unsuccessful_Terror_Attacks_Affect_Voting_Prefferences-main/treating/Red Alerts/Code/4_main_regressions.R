...
coalition_percentage = read.csv('treating/Red Alerts/Output/Processed_Parties/2_Party_coalition_2015_aggregate.csv')
...
reg_1_likud =
  ...
  feols(likud_percentage ~ i(year_election, temporal_group,
        ...
        data = likud_percentage %>% filter(Religion_yishuv_Code != 2),
        se = 'standard')
...
# FIXED EFFECTS
reg_1_coalition =
  feols(party_percentage ~ i(year_election, temporal_group,
                                 ref = '2013', 
                                 ref2 = 'no_red_alert')| 
          as.factor(SEMEL_YISHUV) + as.factor(year_election),
        data = election_percentages %>% filter(Religion_yishuv_Code != 2),
...
# CONTROL VARIABLES + FIXED EFFECTS
reg_2_coalition = 
  feols(coalition_percentage ~ i(year_election, temporal_group,
                                 ref = '2013', 
                                 ref2 = 'no_red_alert') +
          density + Pop_Total + ntl | 
          as.factor(SEMEL_YISHUV) + as.factor(year_election),
