...
election_percentages = election_percentages %>% 
  mutate(
    ...
    year_2006 = ifelse(year == 2006,
                       1,
                       0),
    
    year_2009 = ifelse(year == 2009,
                       1,
                       0),
    

    year_2013 = ifelse(year == 2013,
                   1,
                   0),
    
    year_2015 = ifelse(year == 2015,
                       1,
                       0)
...
reg_1_likud = feols(likud_percentage ~  temporal_group*post,
                     data = election_percentages %>% filter(Religion_yishuv_Code != 2 & (year == 2013  | year == 2015)),
...
reg_2_likud = feols(likud_percentage ~ temporal_group*post +
                distance + density + Shape_Area +Pop_Total + ntl | as.factor(SEMEL_YISHUV),
              data = election_percentages %>% filter(Religion_yishuv_Code != 2),
              cluster = 'SEMEL_YISHUV')

reg_2_likud = feols(likud_percentage ~ i(time_to_treatment, treated, ref = -1)  +
                      distance + density + Shape_Area + Pop_Total + ntl | as.factor(SEMEL_YISHUV) + as.factor(year),
                    data = election_percentages %>% filter(Religion_yishuv_Code != 2),
                    cluster = 'SEMEL_YISHUV')
...
reg_2_right_wing = feols(right_wing_percentage ~  temporal_group*post +
                distance + density + Shape_Area +  ntl  +  Pop_Total | as.factor(SEMEL_YISHUV),
                data = election_percentages %>% filter(Religion_yishuv_Code != 2),
                cluster = 'SEMEL_YISHUV')
...
reg_2_turnout = feols(turnout_percentage ~  temporal_group*post +
                         density + Shape_Area + ntl + Pop_Total | as.factor(SEMEL_YISHUV),
                       data = election_percentages %>% filter(Religion_yishuv_Code != 2),
                      cluster = 'SEMEL_YISHUV')
...
modelsummary(list(reg_likud_60, reg_likud_70, reg_likud_80,
             ...
             coef_map = c('temporal_grouptemporal_distance == 6:postTRUE:pro_likud_60' = '60th percentile - 6 days',
                          ...
                          '(Intercept)' = 'Intercept',
                          'pro_likud_60' = '60th percentile',
                          'pro_likud_70' = '70th percentile',
                          'pro_likud_80' = '80th percentile',
                          'pro_likud_90' = '90th percentile'
...
reg_2_likud_het = feols(likud_percentage ~ temporal_group * post * pro_likud_2013 +
                            Pop_Total + density + Shape_Area + distance + ntl | as.factor(SEMEL_YISHUV),
                        data = election_percentages %>% filter(Religion_yishuv_Code != 2),
                        cluster = 'SEMEL_YISHUV')
