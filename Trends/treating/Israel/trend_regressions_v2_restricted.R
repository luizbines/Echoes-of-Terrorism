...
# Restricting sample for 2014-2015 (election month)
# Luiz Bines
# January 2025
# luizbines@gmail.com
...
trends <-  read.csv('Trends/cleaning/Red Alerts/output/trends_alerts_data_v2.csv', 
                    fileEncoding = "UTF-8") %>% 
  mutate(
  across(-c(District), as.numeric),
  year = as.integer(year),
  month = as.integer(month)
  ) %>%
  # restricting sample up to the 2015 election month
  filter(year == 2014 | year == 2015 & month <=3)
...
walk2(groups, names(groups), ~ {
  modelsummary(
    all_regs[.x],
    output = paste0('Trends/treating/Israel/output/tables/restricted/reg_table_', .y, '_restricted.tex'),
...
walk(all_vars, ~ {
  ...
  file_name <- paste0("Trends/treating/Israel/output/plots/restricted/trend_", .x, "_restricted.pdf")
